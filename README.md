...
- Composite units with `/` (`Мбит/с`, `км/ч`, `об/мин`, `кг/м²`, `руб/шт`, ...) are protected from the math `/` rewriter: the slash is wrapped in U+2060 (Word Joiner) inside a curated list of compound units, so `Мбит/с` doesn't turn into `Мбит / с`. Only triggers after a digit (`100 Мбит/с`); standalone `Мбит/с` without a number is left untouched.
...
- Version strings (`v2.0.0-alpha`, `1.2.3`, `1.2.3-rc1`, `0.10-rc1`) are protected as opaque tokens — `NUM_RANGE_RE` won't turn `2.0` or `0.10` into ranges. Triggers on: `v` prefix (`v\d+.\d+...`), three+ dot-separated parts (`\d+.\d+.\d+...`), or two parts with a letter-starting suffix (`\d+.\d+-[A-Za-z]...`). Ambiguous `1.2-3` (two parts + digit suffix) is intentionally **not** masked — by convention treated as a range.
- `12''` → `12″`, `12'` → `12′` (primes).
- `45 deg` → `45°`.
...
- Em-dashes (`—`) and ASCII hyphens are preserved between letters. The math minus `−` (U+2212) is **not** auto-applied to hyphens between alphanumerics — `Wi-Fi`, `e-mail`, `re-init`, `iPhone 12-pro` stay as-is. Financial minus (`-300 ₽` → `−300 ₽`) is handled by per-language rules, not by math.
