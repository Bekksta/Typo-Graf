...
import { NBSP, EN_DASH, ELLIPSIS, ANY_SPACE_SRC, WORD_JOINER } from "../lang/maps";
...
function protectIsoDates(text: string): string {
  return text.replace(ISO_DATE_RE, (m) => {
    const datePart = m.slice(0, 10).replace(/-/g, `${WORD_JOINER}-${WORD_JOINER}`);
...
// Запускать ПОСЛЕ protectIsoDates: полная ISO-дата уже обёрнута WORD_JOINER,
...
function protectYearMonth(text: string): string {
  return text.replace(YEAR_MONTH_RE, `$1${WORD_JOINER}-${WORD_JOINER}$2`);
}
...
// Шаблон числа покрывает и положительные (`5 кг`), и отрицательные с минусом
// (`-15 %`, `−2 °C`). До smoke-test v1.0.2 #L5 префикс `-/−` не захватывался,
// и регекс не находил `число` слева — пробел между минус-числом и единицей
// оставался обычным, не NBSP.
const COMMON_UNITS_RE = new RegExp(
  `([-−]?\\d+)${ANY_SPACE_SRC}+(` +
    // SI: масса, длина, объём, время (общеупотребительные)
    `кг|мг|г|т|мкм|нм|см|мм|м|км|л|мл|млн|тыс\\.?|` +
    `ч\\.?|мин\\.?|сек\\.?|мкс|нс|мс|` +
    // Углы и температура (язык-агностично; должны идти перед `°` alone)
    `°C|°F|°|` +
    // Валюты
    `₽|€|\\$|£|¥|¢|₸|₴|₪|%|‰|` +
...
const ELLIPSIS_UNITS_RE = new RegExp(
  `…${ANY_SPACE_SRC}+(см\\.?|мм|м|км|г|кг|л|%|₽|€|\\$)`,
  "g"
);

const DOUBLE_PRIME_RE = new RegExp(`(\\d)${ANY_SPACE_SRC}*''(?!')`, "g");
const SINGLE_PRIME_RE = new RegExp(`(\\d)${ANY_SPACE_SRC}*'(?!')`, "g");
...
export function applyCommonRules(input: string): string {
  ...
  text = text.replace(PERCENT_ELLIPSIS_RE, "%" + WORD_JOINER + ELLIPSIS);
