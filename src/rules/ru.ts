...
// Правый lookahead: буквы (RU/EN), цифры, открывающая «, плюс типографские
// знаки, которые читаются как «следующее слово» — №, §, копирайты ©®™,
// валюты ¢£¥€₽$, юникод-минус −. До smoke-test v1.0.0 (bug #1) сюда не
// попадали символы: `по № 5`, `за $ 100`, `до © 2026` оставались с обычным
// пробелом после проклитики.
const PROCLITICS_RE = new RegExp(
  `(?<=^|[\\s(>])(${PROCLITICS.join(
    "|"
  )})(?:${ANY_SPACE_SRC})(?=[A-Za-z\u0410-\u042F\u0430-\u044F\u0401\u04510-9\xAB\xA2\xA3\xA5\xA7\xA9\xAE$\u2116\u20AC\u20BD\u2122\u2212])`,
  "gmi"
);
