...
// Lookahead — буквы (RU+EN) и цифры. Раньше было `(?=\S)`, и одиночная
// `a`/`A`/`i`/`I` рядом с `·`/`→`/`—` в математике подхватывалась как
// артикль и съедала пробел справа: `a · b` превращалось в `a<NBSP>· b`
// (smoke-test v1.0.0 bug #3). Так же — `the` перед замаскированным
// брендом (`the iPhone`) теперь оставляет обычный пробел: брендовые
// маски не должны прилипать к артиклю (поведение симметрично RU).
//
// Регистр перечислен явно в `enLib.PROCLITICS`, флаг `i` снят, иначе
// `A → B` снова матчится как артикль.
const PROCLITICS_RE = new RegExp(
  `\\b(${PROCLITICS.map(escapeRegex).join("|")})${ANY_SPACE_SRC}+(?=[A-Za-zА-Яа-яЁё0-9“‘])`,
  "gm"
);
...
// CMS / Garner's: `e.g.` и `i.e.` пишутся СЛИТНО (без внутреннего пробела).
// German-style `e. g.` (NBSP внутри) до smoke-test v1.0.0 был артефактом
// ранней реализации — см. docs/smoke-test-v1.0.0.md §5. После сокращения
// — NBSP к следующему слову, чтобы аббревиатура не висела в конце строки.
function glueLatinAbbrs(text: string): string {
  // 1) Нормализуем внутренние пробелы: `e. g.` / `e .g.` → `e.g.`
  // (CMS/Garner's: слитно, без внутреннего NBSP). Требуем точки на ОБЕИХ
  // позициях, чтобы случайно не подцепить "egg", "ego" и т. п.
  text = text.replace(/\be\.\s*g\.(?=\s|,|;|:|\)|$)/g, "e.g.");
  text = text.replace(/\bi\.\s*e\.(?=\s|,|;|:|\)|$)/g, "i.e.");
